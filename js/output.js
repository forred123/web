...
var BKClassifyMatrix;//=  new Array();
// 试听成功和试听失败学生名单
...
function sqlOut() {	
	...
	url = url + "?sqlOut=0"
			  ...
			  + "&classInMLS=" + classInMLS
			  + "&timeStart=" + document.getElementsByName("timeStart")[0].value
...
function appendRowBK(mat,index){
	...
	newTd0.innerHTML = index;
	newTd1.innerHTML = mat[0];//obj.schoolZone;
	newTd2.innerHTML = mat[1];//obj.teacher;
	newTd3.innerHTML = mat[2];//obj.grade;
	newTd4.innerHTML = mat[3];//科目
	newTd5.innerHTML = mat[4];//obj.classInMLS;
	newTd6.innerHTML = attandenceDate;
	newTd7.innerHTML = attandenceTimeStart+ '~' +attandenceTimeEnd;
	newTd8.innerHTML = mat[9];//总产出（本次课学生单价的总和）
	newTd9.innerHTML = mat[8]; //出勤人数
	...
	//}else{
		...
		newTd10.innerHTML = testSucNum;// mat[12];///////////////////////////????????????
		if(nameSuc==""){
			newTd11.innerHTML="无";
		}else{
	...
	newTd14.innerHTML = mat[8]-(mat[12]-testSucNum);//本次上课同一个班级的扣费人数
	testSucNum = 0;
...
function loadBKMatrix(obj){
	...
	for(var i=0;i<obj.length;i++){		
		...
		tmpMatrx[i][3] = course;
		tmpMatrx[i][4] = obj[i].className;// 班级名称
		tmpMatrx[i][5] = obj[i].attandenceTime;
		tmpMatrx[i][6] = obj[i].price;
		tmpMatrx[i][7] = obj[i].pay;
		tmpMatrx[i][8] = 1;//本次课学生总
		if(obj[i].priceState=="0"){
			tmpMatrx[i][9] = 0;
		}else{
			tmpMatrx[i][9] = parseFloat(obj[i].price); //总产出（本次课学生单价的总和）
		}
		
		tmpMatrx[i][10] = obj[i].classInMLS;//班级ID
		tmpMatrx[i][11] = obj[i].period;// 班课时长

		if(obj[i].priceState=="0"){
			tmpMatrx[i][12] = 1;// 自己就是试听学生
		}else{
			tmpMatrx[i][12] = 0;// 自己不是试听学生或试听失败的
		}
		
		tmpMatrx[i][13] = obj[i].name1;//扣费学生姓名1
		tmpMatrx[i][14] = obj[i].name2;//扣费学生姓名2
		tmpMatrx[i][15] = obj[i].price;//扣费学生班课单价
		tmpMatrx[i][16] = obj[i].priceState;//扣费学生priceState
