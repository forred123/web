...
function btnTest(studentID, objOperate, type) {
	...
	url = url + "?testStudentID=" + studentID + "&subFeeCourse=" + courseIndex
	// 不使用classIdInMLS，是因为这是个ajax关键词，会触发别的ajax开始查询
	+ "&classInMLS=" + document.getElementsByName("gradeResult")[0].value + "&testResult=" + type + "&priceState=" + priceStateTmp + "&testResultTime=" + document.getElementsByName("attandenceTime")[0].value;
...
function subFeeAllSun() {
	...
	var url = "classRecord.php";
	url = url + "?subFeeStudentID=" + studentID + "&subFeeCourse=" + courseIndex + "&studentName1=" + encodeURIComponent(document.getElementsByName("studentName1")[indexLine].value) + "&studentName2=" + encodeURIComponent(name2) + "&attendance=" + encodeURIComponent(attendance) + "&attandenceTime=" + document.getElementsByName("attandenceTime")[0].value + "&endAttandenceTime=" + document.getElementsByName("endAttandenceTime")[0].value + "&price=" + document.getElementsByName("price")[indexLine].value + "&notAttendanceReason=" + encodeURIComponent(document.getElementsByName("notAttendanceReason")[indexLine].value) + "&schoolZone=" + encodeURIComponent(strData[3]) + "&grade=" + encodeURIComponent(strData[4]) + "&product=" + encodeURIComponent(strData[6]) + "&teacher=" + encodeURIComponent(strData[7]) + "&className=" + encodeURIComponent(strData[8])
	// 不使用classIdInMLS，是因为这是个ajax关键词，会触发别的ajax开始查询
	+ "&classInMLS=" + document.getElementsByName("gradeResult")[0].value + "&pay=" + document.getElementsByName("pay")[indexLine].value + "&priceState=" + priceState;
...
function subFee(studentID, objOperate) {
	...
	if (pass) {
		...
		var url = "classRecord.php";
		url = url + "?subFeeStudentID=" + studentID + "&subFeeCourse=" + courseIndex + "&studentName1=" + encodeURIComponent(document.getElementsByName("studentName1")[index].value) + "&studentName2=" + encodeURIComponent(name2) + "&attendance=" + encodeURIComponent(attendance) + "&attandenceTime=" + document.getElementsByName("attandenceTime")[0].value + "&endAttandenceTime=" + document.getElementsByName("endAttandenceTime")[0].value + "&price=" + document.getElementsByName("price")[index].value + "&notAttendanceReason=" + encodeURIComponent(document.getElementsByName("notAttendanceReason")[index].value) + "&schoolZone=" + encodeURIComponent(strData[3]) + "&grade=" + encodeURIComponent(strData[4]) + "&product=" + encodeURIComponent(strData[6]) + "&teacher=" + encodeURIComponent(strData[7]) + "&className=" + encodeURIComponent(strData[8])
		// 不使用classIdInMLS，是因为这是个ajax关键词，会触发别的ajax开始查询
		+ "&classInMLS=" + document.getElementsByName("gradeResult")[0].value + "&pay=" + document.getElementsByName("pay")[index].value + "&priceState=" + priceState;
...
function outGrade(studentID, objOperate) {
	...
	var url = "classRecord.php";
	url = url + "?outGradeStudentID=" + studentID + "&outOrBackGradeTime=" + document.getElementsByName("attandenceTime")[0].value + "&outGradeReason=" + document.getElementsByName("outGradeReason")[index].value + "&course=" + document.getElementsByName("operateCourse")[0].value;
...
function backGrade(studentID, objOperate) {
	...
	var url = "classRecord.php";
	url = url + "?backGradeStudentID=" + studentID + "&course=" + document.getElementsByName("operateCourse")[0].value;
	// 很重要，必须有的
	url = url + "&sid=" + Math.random();
	xmlhttp.open("GET", url, true);
	xmlhttp.send(null);
...
function loadStudentInOneGrade(flag) {
	...
	var url = "classRecord.php";
	url = url + "?classIdInMLS=" + document.getElementsByName("gradeResult")[0].value + "&stateInGrade=" + stateInGrade + "&course=" + document.getElementsByName("operateCourse")[0].value;
	//alert(url);
	// 很重要，必须有的
	url = url + "&sid=" + Math.random();
...
function appendRow(TableID, studentID, name1, name2, startTime, endTime, stateInGrade, outGradeReason, notAttendanceReason, attendance, price, pay, remainFee, index) {
	...
	// 班课时不显示工资
	if (strData[6] == "班课") {
		newTd7.innerHTML = '<input type="number" name="pay" style="width: 4em"' + ' readonly/>';
		//工资
	} else {
		newTd7.innerHTML = '<input type="number" name="pay" style="width: 4em" value=' + pay + ' />';
		//工资
	}
	...
	newTd9.innerHTML = '<select name="attendance" style="width: 5em" onchange=freshAttandence()>' + '<option value="2">出勤</option>' + '<option value="1">请假</option>' + '<option value="0" style="display: none" >旷课</option>' + '</select>';
	if (attendance == "旷课") {
	...
	if ((stateInGrade == "0") || (stateInGrade == "4")) {
		newTd13.innerHTML = '<input type="text" name="outGradeReason" style="width: 6ex" value=' + outGradeReason + ' readonly/>' + '<input type="button" class="btn" name="outGradeName" value="退班" onclick="outGrade(' + studentID + ', this )" disabled />';
		...
		newTd13.innerHTML = '<input type="text" name="outGradeReason" style="width: 6ex" />' + '<input type="button" class="btn" name="outGradeName" value="退班" onclick="outGrade(' + studentID + ', this )"/>';
	...
	if (stateInGrade == "2") {
		newTd15.innerHTML = '<input type="hidden" name="testState" value="试听"/>' + '<input type="button" name="btnTestYes" value="试听成功" onclick="btnTest(' + studentID + ', this ,3)"/>' + '<input type="button" name="btnTestNo" value="试听失败" onclick="btnTest(' + studentID + ', this ,4)"/>';
	} else {
		newTd15.innerHTML = '<input type="hidden" name="testState" value="非试听"/>' + '<input type="button" name="btnTestYes" value="试听成功" onclick="btnTest(' + studentID + ', this ,3)" disabled/>' + '<input type="button" name="btnTestNo" value="试听失败" onclick="btnTest(' + studentID + ', this ,4)" disabled/>';
	}

	var str1 = dateJS("Y-M-d_H:i", startTime);
	var str2 = dateJS("Y-M-d_H:i", endTime);

	newTd2.innerHTML = '<input type="text" name="startTime" value=' + str1 + ' style="width: 18ex" readonly="true"/>' + "</br>" + '<input type="text" name="endTime" value=' + str2 + ' style="width: 18ex" readonly="true"/>';
...
function loadPriceAndAddRow(obj, flagCourse, remainFee, i) {
	...
	// 3发出http请求
	var url = "classRecord.php";
	url = url + "?sqlPriceByUid=" + obj.uid + "&course=" + flagCourse.toString + "&product=" + strtmpGrade[6];
